...
let rec countIterationsTillRepeat =
        (iterations: int, currentIteration: blocks, seen: list(blocks))
        : int =>
  if (List.mem(getNextIteration(currentIteration), seen)) {
    Js.log(getNextIteration(currentIteration));
    iterations + 1;
  } else {
    countIterationsTillRepeat(
      iterations + 1,
...
Js.log(countIterationsTillRepeat(0, parsedInput, [parsedInput]));